...
IndexQueue<Capacity>::print() const
{
	...
	auto wPos = m_tail.load(std::memory_order_acquire);
	auto rPos = m_head.load(std::memory_order_acquire);
	cout
	<< "rPos head[" << rPos.getCycle() << ", " << rPos.getIndex() << "]" << endl
	<< "wPos tail[" << wPos.getCycle() << ", " << wPos.getIndex() << "]" << endl
	;
	for(NativeType i=0; i<Capacity; ++i){
		auto idx = m_values[i].load(std::memory_order_acquire);
...
void IndexQueue<Capacity>::push(indexvalue_t index, MonitoringPolicy const& policy)
{
    Index oldTail;
    do
    {
